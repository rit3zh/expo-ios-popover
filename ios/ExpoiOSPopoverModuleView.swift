...
public final class ExpoPopoverView: ExpoView {
    ...
    var triggerType: String = "tap" {
        didSet {
            if let trigger = triggerView {
    ...
    private func setupTapGesture(on view: UIView) {
        ...
        case "longpress":
            gesture = UILongPressGestureRecognizer(target: self, action: #selector(handleTap(_:)))
        case "doubletap":
            let dt = UITapGestureRecognizer(target: self, action: #selector(handleTap(_:)))
    ...
    @objc private func handleTap(_ sender: UIGestureRecognizer) {
        ...
        parentVC.present(controller, animated: animated) { [weak self] in
...
public final class ExpoPopoverPressableView: ExpoView {
